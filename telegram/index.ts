...
function point(x: number, y: number, d = 3) {
...
function line(xs: number, ys: number, xe: number, ye: number) {
    ctx.beginPath()
    ctx.moveTo(xs, ys)
    ctx.lineTo(xe, ye)
    ctx.stroke()
...
function graph(mtx: Array<number>, legend = true) {
    const m = 10
    ...
    { // graph
        function _graph(column: [any, ...any[]]) {
...
function slider(y: number, h: number, w: number, percent = 0.25) {
